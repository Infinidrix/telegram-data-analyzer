...
const date_parser = (message, stats, temp_stats, sender) => {
    ...
    date_stats[message_date.year()].all.all++;
    ...
    date_stats[message_date.year()][message_date.month()].all.all++;
}
...
const daily_parser = (message, stats, temp_stats, sender) => {
    ...
    daily_stats[message_date.dayOfYear()].all.all++;
...
const week_parser = (message, stats, temp_stats, sender) => {
    ...
    if (week_stats[message_date.weekday()] === undefined){
        week_stats[message_date.weekday()] = {all: {[sender]: 0, all: 0}};
    ...
    week_stats[message_date.weekday()].all.all++;
...
const hour_parser = (message, stats, temp_stats, sender) => {
    ...
    if (hour_stats[message_date.hour()] === undefined){
        hour_stats[message_date.hour()] = {all: {[sender]: 0, all: 0}};
    ...
    hour_stats[message_date.hour()].all.all++;
}
